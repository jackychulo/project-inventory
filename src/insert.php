<?php
    ...
    if($stmt = $conn->prepare($sql)){
        ...
        if($stmt->execute()){
            $resultarray['response'] = "Submission Successful: Name >> ". $name;
        }else{
            $resultarray['response'] = "Submission Failed: name >> ". $name . "</br>" . $stmt->error;
        }
    }else{
        $resultarray['response'] = "Submission Failed: Error: ". $conn->error;
    }
    $conn->close();
